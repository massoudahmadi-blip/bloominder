...
export async function getSalesInView(bbox: BBox, filters: Filters): Promise<MapView> {
  ...
  const sales = (fc.features ?? []).map((feat: any, i: number): Sale => ({
    ...
    id_parcelle: feat.properties.id_parcelle ?? null,
    section: feat.properties.section ?? null,
    surface_carrez: feat.properties.surface_carrez ?? null,
    nombre_lots: feat.properties.nombre_lots ?? null,
    geo_precision: feat.properties.geo_precision ?? null,
