...
export function mockScreener(sort?: ScreenerSort, dir: 'asc' | 'desc' = 'desc'): CommuneRow[] {
  const rows: CommuneRow[] = COMMUNES.map((c, i) => {
    ...
    return {
      ...
      median_prix_m2_12m: Math.round(med * 1.02),
      prix_m2_growth_1y: 2 + i * 0.8,
...
export function mockCommune(code: string): CommuneProfile | null {
  ...
  return {
    metrics: {
      ...
      median_prix_m2_appartement: med,
      median_prix_m2_maison: Math.round(med * 0.95),
      median_prix_m2_12m: Math.round(med * 1.03),
      prix_m2_growth_1y: 4.2,
    ...
    demo: { population: 52000, pop_growth: null, median_income: 22500 },
    ...
    livability: { schools: 28, ecoles: 20, colleges: 5, lycees: 3, education_prioritaire: true, crime_rate: 34.2, health_equip: 64, transport_equip: 12, fiber_pct: 88.5 },
